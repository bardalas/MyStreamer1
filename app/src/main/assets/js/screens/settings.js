...
const PANES = {
  ...
  look: () => {
    ...
    return section(tr('set.skin.title'), `<div class="themes">${SKINS.map(themeCard).join('')}</div>`)
      + section(tr('set.skin.custom'), `<div class="colorpickers">
        <label><span>${tr('set.skin.bg')}</span><input type="color" data-col="bg" value="${esc(col.bg)}"></label>
        <label><span>${tr('set.skin.accent')}</span><input type="color" data-col="accent" value="${esc(col.accent)}"></label>
        <label><span>${tr('set.skin.secondary')}</span><input type="color" data-col="secondary" value="${esc(col.secondary)}"></label>
      </div>`);
  },
  live: () => {
...
function wire(pane){
  ...
  pane.querySelectorAll('[data-col]').forEach(input => input.oninput = () => {
    setSetting('customColors', {...(settings.customColors || {}), [input.dataset.col]: input.value});
    setSetting('skin', 'custom');
    pane.querySelectorAll('.theme').forEach(x => x.classList.toggle('on', x.dataset.skin === 'custom'));
  });
