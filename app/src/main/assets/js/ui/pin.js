...
import {checkPin, failSum, grownUpSum, hasPin, pinLockedFor, setPin} from '../data/kids.js';
...
export async function askPin(title){
...
/** A grown-up's say-so for [title]: the code when there is one; with none set yet, nothing to ask. */
export const grownUp = title => hasPin() ? askPin(title) : Promise.resolve(true);
