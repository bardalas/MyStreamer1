...
/* A menu that opens over the page hides the very thing the viewer is looking at, so opening it moves
   ...
   Whether the menu is open is ONE state - body.railwide - worked out in ONE place from what is true now: the
   focus is inside it, or a mouse (a pointer, an air remote) moved over it a moment ago. The stylesheet reads only
   that class. A pointer that merely stays where it was left - after a tap, after a click that took a title away - is
   not "in" the menu, and focus that has gone elsewhere is not either; the state is recomputed at every change
   that could matter (focus, pointer, screen, key, something in the menu added or removed). */
const rail = $('#rail');
const POINTER_MS = 1200;
let pointerOver = false, lastMove = 0, idle = 0, queued = 0;
const wanted = () => !!rail?.contains(document.activeElement) || (pointerOver && performance.now() - lastMove < POINTER_MS);
const apply = () => {
  const on = RAIL_MQ.matches && wanted();
  if(document.body.classList.contains('railwide') === on) return;
  document.body.classList.toggle('railwide', on);
  setTimeout(() => dispatchEvent(new Event('resize')), 400);
};
const recheck = () => { clearTimeout(queued); queued = setTimeout(apply, 30); };      // after the focus has settled where it is going
rail?.addEventListener('pointermove', e => {
  if(e.pointerType === 'touch') return;                        // a finger is not hovering: its focus says when it is in
  pointerOver = true; lastMove = performance.now(); apply();
  clearTimeout(idle); idle = setTimeout(recheck, POINTER_MS + 50);
});
rail?.addEventListener('pointerleave', () => { pointerOver = false; recheck(); });
addEventListener('focusin', recheck);
addEventListener('focusout', recheck);
addEventListener('hashchange', recheck);
addEventListener('keydown', recheck, true);
RAIL_MQ.addEventListener?.('change', recheck);
if(rail) new MutationObserver(recheck).observe(rail, {childList: true, subtree: true});
